...
import { Plus, Layers, ShoppingBag, LayoutTemplate, Percent, ArrowUpRight, Loader2 } from "lucide-react";
import { PageHeader, Panel, Stat, Pill, statusTone } from "@/components/admin/kit";
import { KES, assetUrl } from "@/lib/format";
import { useDashboardData, useCustomerStats, useRecentCustomers, useProductStatusCounts } from "@/lib/queries/dashboard";
...
function Dashboard() {
  ...
  const { data: dash, isLoading: dashLoading } = useDashboardData(range);
  const { data: customerStats } = useCustomerStats();
  const { data: recentCustomers = [] } = useRecentCustomers();
  const { data: productStatus } = useProductStatusCounts();
      ...
      {dashLoading || !dash ? (
        ...
        <>
          <div className="grid gap-px bg-border sm:grid-cols-2 xl:grid-cols-4">
            <Stat label="Total sales" value={KES(dash.totalSales)} delta={dash.salesDelta} hint={`vs previous · ${range}`} />
            <Stat label="Orders" value={String(dash.orderCount)} delta={dash.ordersDelta} hint={`${dash.pendingCount} awaiting action`} />
            <Stat label="Revenue (net)" value={KES(dash.netRevenue)} delta={dash.netRevenueDelta} hint="after refunds/cancellations" />
            <Stat label="Average order value" value={KES(dash.aov)} delta={dash.aovDelta} hint="per order" />
            <Stat
              label="Products"
              value={String(productStatus?.total ?? 0)}
              hint={`${productStatus?.published ?? 0} published · ${productStatus?.draft ?? 0} draft`}
            />
            <Stat
              label="Customers"
              value={String(customerStats?.total ?? 0)}
              delta={customerStats?.newThisMonth ? `+${customerStats.newThisMonth}` : undefined}
              hint="new this month"
            />
            <Stat label="Low-stock products" value={String(lowStock.length)} hint="at or below threshold" />
            <Stat label="Pending orders" value={String(dash.pendingCount)} hint="need confirmation" />
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            <Panel title="Revenue over time" className="lg:col-span-2">
              <ResponsiveContainer width="100%" height={260}>
                <AreaChart data={dash.revenueSeries} margin={{ left: -18, right: 8, top: 8 }}>
                  <defs>
            ...
            <Panel title="Sales by category">
              {dash.categorySales.length === 0 ? (
                <p className="py-16 text-center text-sm text-muted-foreground">No sales in this period yet.</p>
              ) : (
                <>
                  <ResponsiveContainer width="100%" height={260}>
                    <PieChart>
                      <Pie
                        data={dash.categorySales}
                        dataKey="value"
                        nameKey="name"
                        innerRadius={55}
                        outerRadius={90}
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            <Panel title="Orders over time" className="lg:col-span-2">
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={dash.revenueSeries} margin={{ left: -22, right: 8, top: 8 }}>
                  <CartesianGrid stroke="oklch(0.9 0.008 80)" vertical={false} />
                  <XAxis dataKey="day" tickLine={false} axisLine={false} fontSize={11} />
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            >
              {dash.recentOrders.length === 0 ? (
                <p className="py-8 text-center text-sm text-muted-foreground">No orders in this period yet.</p>
              ) : (
